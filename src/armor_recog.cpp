/*
TODO: modify the $thres_max_color_red&blue & $binary_brightness according to the camera parameters
...
void Armor_recog::pre_process() {
    ...
    namedWindow("thresh_whole", WINDOW_FREERATIO);
    namedWindow("subtracted1", WINDOW_FREERATIO);
    namedWindow("subtracted2", WINDOW_FREERATIO);
    namedWindow("dilated", WINDOW_FREERATIO);
    namedWindow("dilated again", WINDOW_FREERATIO);
...
//get the angle of first edge to horizon
                ...
                bool isLengthWidthVertical = (angle_edge + angle_center) > 70 && (angle_edge + angle_center) < 110;
                bool isDeltaAngleValid = deltaAngle < DELTA_ANGLE_THRESHOLD;
                bool isAPanel = (length_width_ratio_panel > STANDARD_LENGTH_WIDTH_RATIO * 0.5) && (length_width_ratio_panel < STANDARD_LENGTH_WIDTH_RATIO * 1.5);
                if(isDeltaAngleValid && isAPanel && isLengthWidthVertical) {
